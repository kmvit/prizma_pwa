...
Отправка сообщений и файлов пользователям (логика 1:1 из perplexy_bot)
"""
...
class TelegramService:
    ...
    def __init__(self):
        ...
        if not self.bot_token:
            logger.warning("⚠️ TELEGRAM_BOT_TOKEN не настроен, отправка в Telegram отключена")
            self.enabled = False
        else:
            self.enabled = True
...
🔥 <b>Хотите получить по акции?</b>
            ...
            price_offer_url = f"{self.webapp_url}/offer" if self.webapp_url else ""
                ...
                with open(photo_path, 'rb') as file:
                    ...
                    async with session.post(url, data=data) as response:
                        if response.status == 200:
                            result = await response.json()
                            if result.get("ok"):
                                logger.info(f"✅ Изображение с клавиатурой отправлено пользователю {chat_id}")
                                return True
                            else:
...
• 📈 Индивидуальный план развития с упражнениями и ресурсами для устойчивых изменений
                ...
                "inline_keyboard": [[
                    {
                        ...
                        "web_app": {"url": f"{self.webapp_url}/offer"}
                    }
                ]]
            }
            success = await self.send_message_with_keyboard(telegram_id, message, keyboard)
            ...
            logger.error(f"❌ Ошибка при отправке уведомления за 6 часов пользователю {telegram_id}: {e}")
            return False
...
🧭 Эта книга-анализ станет вашим личным путеводителем на пути к осознанной жизни, росту и процветанию
            ...
            logger.error(f"❌ Ошибка при отправке уведомления за 1 час пользователю {telegram_id}: {e}")
            return False
...
Вы больше не сможете получить полную расшифровку со скидкой –50% ☹️
            ...
            logger.error(f"❌ Ошибка при отправке уведомления за 10 минут пользователю {telegram_id}: {e}")
            return False
    ...
    def _compose_link_message(self, is_premium: bool, url: str) -> str:
        ...
        if is_premium:
            report_word = "Премиум-отчет"
        else:
            report_word = "Отчет"
        link_text = "Это ссылка для скачивания вашей расшифровки. Нажмите, чтобы ее скопировать и вставить в браузер"
        return (
